...
CREATE TABLE IF NOT EXISTS `pet_agenda`.`servico_disponivel` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_id_tipo_servico_idx` (`id_tipo_servico` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `pet_agenda`.`cliente` (
  ...
  INDEX `fk_id_servico_solicita_idx` (`id_servico_solicita` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `pet_agenda`.`usuario` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_id_endereco_idx` (`id_endereco` ASC) VISIBLE,
  INDEX `fk_id_servico_presta_idx` (`id_servico_presta` ASC) VISIBLE,
  INDEX `fk_id_permissao_idx` (`id_permissao` ASC) VISIBLE,
  INDEX `fk_id_local_atuacao_idx` (`id_local_atuacao` ASC) VISIBLE,
  CONSTRAINT `fk_id_endereco`
    FOREIGN KEY (`id_endereco`)
    REFERENCES `pet_agenda`.`endereco` (`id`)
    ON DELETE NO ACTION
...
CREATE TABLE IF NOT EXISTS `pet_agenda`.`pet` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_id_cliente_dono_idx` (`id_cliente_dono` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `pet_agenda`.`agendamento` (
  ...
  `id_tipo_servico` INT NOT NULL,
  `dh_hr_marcada` DATETIME NOT NULL,
  `id_pet` INT NOT NULL,
  ...
  INDEX `fk_id_pet_idx` (`id_pet` ASC) VISIBLE,
  INDEX `fk_id_funcionario_agend_idx` (`id_funcionario_agend` ASC) VISIBLE,
  INDEX `fk_id_pacote_agendamento_idx` (`id_pacote_agendamento` ASC) VISIBLE,
  INDEX `fk_id_id_tipo_servico_idx` (`id_tipo_servico` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `pet_agenda`.`incidente` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_id_pet_idx` (`id_pet` ASC) VISIBLE,
  INDEX `fk_id_servico_contratado_idx` (`id_servico_contratado` ASC) VISIBLE,
  CONSTRAINT `fk_id_pet`
    FOREIGN KEY (`id_pet`)
    REFERENCES `pet_agenda`.`pet` (`id`)
    ON DELETE NO ACTION
...
CREATE TABLE IF NOT EXISTS `pet_agenda`.`historico_servico` (
  `id_agendamento` INT NOT NULL,
  `descricao` TEXT NULL,
  `dthr_finalizado` DATETIME NOT NULL,
  ...
  INDEX `fk_id_incidente_idx` (`id_incidente` ASC) VISIBLE,
  CONSTRAINT `fk_id_agendamento`
    FOREIGN KEY (`id_agendamento`)
    REFERENCES `pet_agenda`.`agendamento` (`id`)
    ON DELETE NO ACTION
...
CREATE TABLE IF NOT EXISTS `pet_agenda`.`anexo_historico` (
  ...
  PRIMARY KEY (`id_agendamento`, `caminho_anexo`),
  CONSTRAINT `fk_id_agendamento`
    FOREIGN KEY (`id_agendamento`)
    REFERENCES `pet_agenda`.`agendamento` (`id`)
    ON DELETE NO ACTION
